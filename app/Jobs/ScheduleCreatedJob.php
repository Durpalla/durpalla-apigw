...
class ScheduleCreatedJob implements ShouldQueue
{
    ...
    {
        try {
            $existingCabinIds = ScheduleCabinMapping::where('schedule_id', $this->schedule->id)
                ->pluck('cabin_id')
                ->all();

            $items = Cabin::where('vehicle_id', $this->schedule->vehicle_id)
                ->when($existingCabinIds !== [], fn ($q) => $q->whereNotIn('id', $existingCabinIds))
                ->get();
