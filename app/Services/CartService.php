...
class CartService
{
    ...
    private function resolveCustomerToken(): ?string
    {
        ...
        if ($user) {
            $token = $this->customerTokenForUser($user);
            if ($token !== null) {
    ...
    /**
     * Stable cart lock token for an authenticated user (email, else mobile, else id).
     */
    private function customerTokenForUser(object $user): ?string
    {
        if (! empty($user->email)) {
            return base64_encode((string) $user->email);
        }
        if (! empty($user->mobile)) {
            return base64_encode('mobile:'.(string) $user->mobile);
        }
        if (isset($user->id)) {
            return base64_encode('customer:'.(string) $user->id);
        }
