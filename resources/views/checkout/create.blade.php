...
<div class="container">
    <div id="Checkout" class="inline">
        <h1>
            ...
            <img src="{{ asset('default/logo-icon.png') }}">
        </h1>
        {{--        <div class="card-row">--}}
        ...
        {{--        </div>--}}
        <form method="POST" action="{{ route('checkout.token') }}" id="PaymentForm">
            @csrf
                ...
                <div class="row">
                    <div class="col-xs-6">
                        ...
                        <div class="amount-placeholder">
                            <span>{{ getOption('currency', 'Tk.') }}</span>
                    ...
                    <div class="col-xs-6 text-right">
                        @php
                            $incentive = 0;
                        @endphp
                        @if($order->officer->hasRole('agent'))
                            <label for="PaymentAmount">Incentive</label>
                            <div class="amount-placeholder">
                                <span>{{ getOption('currency', 'Tk.') }}</span>
                                <span>
                        @php
                        ...
                        @else
                            <label for="PaymentAmount">Discount</label>
            ...
            <ul class="list-group gateway-list">
                @php $defaultGateway = 'Bkash'; @endphp
                @foreach($gateway_list as $key => $gateway)
                    @php
                        $defaultGateway = $gateway->name;
                        $logo = ($gateway->logo === null) ? asset('default/' . strtolower($gateway->name). '.jpg') : asset($gateway->logo);
                    @endphp
                    <li class="list-group-item">
                        <!--RADIO 1-->
                        {{--                        <input type="radio" class="radio_item gateway-item" @if($key === 0 ) checked @endif value="{{ $gateway->name }}"--}}
                        {{--                               name="payment_method"--}}
                        {{--                               id="{{ $gateway->name }}">--}}
                        <label class="label_item" for="{{ $gateway->name }}">
                            @if($gateway->name === 'Nagad')
                                <a href="{{ route('nagad.checkout', $order->id) }}"
                                   id="{{ strtolower($gateway->name) }}"><img src="{{ $logo }}"></a>
                            @else
                                <img id="{{ strtolower($gateway->name) }}" data-id="{{ $order->id }}" src="{{ $logo }}">
                            @endif
                        </label>
                    </li>
                @endforeach
            ...
            {{--                </div>--}}
            {{--            </div>--}}
            <button id="bKash_button" type="button" class="hide">Bkash button</button>
            {{--            <button class="btn btn-block btn-success submit-button @if($defaultGateway !== 'Bkash') hide @endif" type="submit" id="PayButton">--}}
            {{--                <span class="submit-button-lock"></span>--}}
            {{--                <span class="align-middle">Pay {{ number_format($order->total_payable - $incentive, 2) }}</span>--}}
            {{--            </button>--}}
            <button class="btn btn-block btn-success submit-button @if($defaultGateway !== 'Sslcommerz') hide @endif"
                    type="submit"
                    id="sslczPayBtn"
                    token="{{ csrf_token() }}"
                    order="{{ $order->id }}"
                    endpoint="{{ route('checkout.token') }}"
            >
                <span class="submit-button-lock"></span>
                <span class="align-middle">Pay {{ number_format($order->total_payable - $incentive, 2) }}</span>
            </button>
    ...
    jQuery(function ($) {
        ...
        $(document).ready(function () {
            ...
            let sslObj = {
                ...
                cus_addr1: "{{ $order->customer['address'] }}",
                amount: {{ $order->total_payable - $incentive }}
            };
            {{--$('.gateway-item').change(function () {--}}
            {{--    if ($(this).is(':checked') && $(this).val() == 'Sslcommerz') {--}}
            {{--        let obj = {};--}}
            {{--        obj.cus_name = "{{ $order->customer['name'] }}";--}}
            {{--        obj.cus_phone = "{{ $order->customer['mobile'] }}";--}}
            {{--        obj.cus_email = "{{ $order->customer['email'] }}";--}}
            {{--        obj.cus_addr1 = "some address";--}}
            {{--        obj.amount = "{{ $order->total_payable }}";--}}
            {{--        console.log(obj);--}}
            {{--        $('#sslczPayBtn').attr('postdata', obj).removeClass('hide');--}}
            {{--        $('#PayButton').addClass('hide');--}}
            {{--    } else {--}}
            {{--        $('#sslczPayBtn').addClass("hide");--}}
            {{--        $('#PayButton').removeClass("hide").removeAttr('disabled');--}}
            {{--    }--}}
            {{--});--}}
            $('#bkash').click(function () {
                ...
                alert(orderID)
                $.ajax({
                    url: "{{ route('checkout.token') }}",
                    type: 'POST',
                    dataType: "json",
                    data: {order: orderID, payment_method: 'Bkash'},
            ...
            function initBkash(data) {
                console.log(data);
                bKash.init({
                    paymentMode: 'checkout',
                    paymentRequest: {amount: data.amount, intent: 'sale'},
                    ...
                    executeRequestOnAuthorization: function () {
                        $.ajax({
                            ...
                            type: 'POST',
                            data: {token: data.token, payment_method: 'Bkash', paymentID: paymentID},
                            dataType: "json",
                            success: function (data) {
                                if (data && data.merchantInvoiceNumber != null) {
                                    window.location.href = "/checkout/success/" + data.merchantInvoiceNumber;
                                } else {
                            ...
                            onClose: function () {
                                alert('You close the payment modal')
                                $('#bKash_button').removeAttr('disabled');
                            }
                        });
                    }
