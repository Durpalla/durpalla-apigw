...
@section('content')
    ...
    <section class="content">
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            ...
            @if(auth()->user()->hasAnypermission(['agent-create']) || auth()->user()->hasRole('admin'))
                <li class="nav-item">
                    <a class="nav-link" href="{{ route('agent.create') }}"><i class="fa fa-plus"></i> Add new
                        agent</a>
                </li>
            @endif
        </ul>
        ...
        $(function () {
            ...
            var table = $('#dataTable').DataTable({
                ...
                "columns": [
                    ...
                    {
                        "mRender": function (data, type, row) {
                            return '<a href="/admin/agent/' + row['id'] + '" class="table-avatar">' + row['name'] + '</a>';
                        }
                    },
                    {"data": "mobile"},
                    {"data": "email"},
                    ...
                    {"data": "status"},
                    {
                        "mRender": function (data, type, row) {
                            var str = "<div class='btn-group'> <button class='btn btn-secondary btn-sm dropdown-toggle' type='button' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'><i class='fa fa-ellipsis-h' aria-hidden='true'></i></button> <div class='dropdown-menu dropdown-menu-right'> " +
                                "<a href='/admin/agent/" + row['id'] + "' class='dropdown-item' data-merchant-id='" + row['id'] + "'><i class='fa fa-eye'></i> View</a> " +
                                "<a href='/admin/agent/" + row['id'] + "/edit' class='dropdown-item'><i class='fa fa-edit'></i> Edit</a>";
