...
@section('content')
    ...
    <section class="content content-overlap">
        <form method="post" action="{{ route('parties.update', $party->id)}}" role="form">
            @csrf
            @method('PUT')
                ...
                <div class="col-7">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" value="{{ old('name', $party->name) }}" name="name"
                                   placeholder="Name" required/>
                        </div>
                        <div class="form-group">
                            <label>Slug</label>
                            <input type="text" class="form-control" value="{{ old('slug', $party->slug) }}" name="slug"
                                   placeholder="Slug" required/>
                        </div>
                        <div class="form-group">
                            <label>Description</label>
                            <textarea class="form-control" name="description"
                                      style="min-height: 80px;">{{ old('description', $party->description) }}</textarea>
                        </div>
                        <div class="form-group">
                            <label>Address</label>
                            <textarea class="form-control" name="address"
                                      style="min-height: 80px;">{{ old('address', $party->address) }}</textarea>
                        </div>
                    </div>
                </div>
                <div class="col-5">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control"
                                   value="{{ old('domain_name', $party->domain_name) }}" name="domain_name"
                                   placeholder="Domain name" required/>
                        </div>
                        <div class="form-group">
                            <label>Mobile</label>
                            <input type="text" class="form-control" value="{{ old('mobile', $party->user['mobile']) }}"
                                   name="mobile"
                                   placeholder="Mobile" required/>
                        </div>
                        <div class="form-group">
                            <label>Email</label>
                            <input type="email" class="form-control" value="{{ old('email', $party->user['email']) }}"
                                   name="email"
                            ...
                            @foreach($service_dropdowns as $key => $value)
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" name="services[]" type="checkbox"
                                           id="inlineCheckbox{{$key}}" value="{{$key}}" checked>
